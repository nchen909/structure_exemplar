...
def add_args(parser):
    ...
    #######################注意改成真随机！！！！！！
    parser.add_argument("--woast", default=0, type=int,
                    help="without ast init,保留GAL模块,表征不dfs遍历的顺序,邻接矩阵全0初始化 ")

    parser.add_argument("--disturb_strategy", default='none', type=str,
                    help="how to disturb (on train dataset)",choices=['none','identifier'])
    parser.add_argument("--calibrate_cls", default=0, type=int,
                    help="calibrate_cls or not ")
    ...
    parser.add_argument("--origin_model_dir", default='/data/huggingface_models/', type=str,
                    help="directory to store origin model")

    parser.add_argument("--sample_rate", default=-1, type=float,
                help="percentage of few_shot, you can either use few_shot to set num or sample_rate to set percentage")

    parser.add_argument("--exemplar_sample_num", default=-1, type=int,
                help="sample top exemplar_sample_num exemplar_strategy exemplar")
    parser.add_argument("--exemplar_strategy", default='l', type=str,
                help="GE for global efficiency, l=1/GE",choices=['ge','l'])
    parser.add_argument("--exemplar_saving_dir", default='data/exemplar', type=str,
                help="dir to save exemplars")
...
def set_hyperparas(args):
    ...
    #     args.few_shot = -1
    # if args.model_name in ['bart', 'plbart']:
    ...
    #     lr=5e-5# 5e-5(in repo)#1e-4(in plbartpaper)
    ...
    #         args.num_train_epochs = 50 if not torch.cuda.is_available() else 50*torch.cuda.device_count()#60
    #         args.patience = min( 20, args.num_train_epochs//5*2)#min( 10, args.num_train_epochs//5*2)
    #     else:#refine
    #         args.num_train_epochs = 60 if not torch.cuda.is_available() else 60*torch.cuda.device_count()#50 in plbart finetune
    #         args.patience = min( 24, args.num_train_epochs//5*2)#min( 10, args.num_train_epochs//5*2)
    #     if args.model_name in ['t5', 'codet5']:
    ...
    #     if args.task in ['summarize','translate','generate','defect']:#3090
    #         if args.prefix_tuning:
    #             args.batch_size = args.batch_size // 2 #4
    #         else:
    #             args.batch_size = args.batch_size // 2
    ...
    #         if args.prefix_tuning:
    #             args.batch_size = args.batch_size // 2 #4
    #     if args.task in ['summarize','generate','defect','clone','refine']:
    #         if args.sub_task == 'medium':
    #             args.batch_size = args.batch_size // 2
    #         #####################################################
    #     elif args.task in ['translate']:
    #         args.batch_size *= 1#####################################################
    #     elif args.task in ['1']:
    #         args.batch_size //= 2#####################################################
    #     elif args.task in ['1']:
