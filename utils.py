...
#is_sample:eval=true,  test=false
def load_and_cache_gen_data(args, filename, pool, tokenizer, split_tag, only_src=False, is_sample=False):
    ...
    if split_tag =='train' and args.disturb_strategy!= 'none':
        ...
        print('disturbed examples[0]:')
        print(examples[0].target if args.task=='generate' else examples[0].source)
    if split_tag =='train' and args.exemplar_sample_num!= -1:
        examplar_ = Exemplar(args,examples)
        examples=examplar_.process_examples()
        print('examplars[0]:')
        print(examples[0].target if args.task=='generate' else examples[0].source)
    ...
    if (split_tag!='test' and is_sample) or (split_tag=='train' and args.few_shot != -1) :
        if args.few_shot <= len(examples):
            ...
            examples = random.sample(examples, sample_num if (args.few_shot == -1 or is_sample) else args.few_shot)
        else:
            args.few_shot = len(examples)
            # # for CodeTrans dataset, dev&test example len = 500, may smaller than few-shot case
            # # we compensate some examples from train set to fill examples to args.few_shot
            # examples_train = read_examples(args.train_filename, -1, args.task)
            # examples += random.sample(examples_train, args.few_shot - len(examples))
...
# class Disturb():
#     def __init__(self, args,examples, disturb_percentage=50):
#         self.args = args
...
def get_retrieve_id_list(args, examples, data, k=1):
    # for num in range(500000,1000000,100000):
    #     print(num)
    #     tokens_list, tokens_type_list = get_retriever_metadata(args,examples[num:num+100000], data)
    #     from retriever.BM25 import BM25
    #     bm25 = BM25([i.values() for i in tokens_type_list])
    #     freq_types,bmscore=bm25.get_freq_word(25)#(15)
    #     print("freq_token_types:\n{} bm25:\n{} \nfor task:{}, lang:{}".format(freq_types,bmscore,args.task, args.lang ))


    tokens_list, tokens_type_list = get_retriever_metadata(args,examples, data)
    from retriever.BM25 import BM25
    bm25 = BM25([i.values() for i in tokens_type_list])
    freq_types,bmscore=bm25.get_freq_word(15)#(15)
    print("freq_token_types:\n{} bm25:\n{} \nfor task:{}, lang:{}".format(freq_types,bmscore,args.task, args.lang ))
...
def load_and_cache_clone_data(args, filename, pool, tokenizer, split_tag, is_sample=False):
    ...
    if (args.calibrate_cls or split_tag=='train') and args.few_shot!=-1:
    ...
    if split_tag =='train' and args.exemplar_sample_num!= -1:
        ...
        print('examplars[0]:')
        print(examples[0].target if args.task=='generate' else examples[0].source)
    if split_tag =='train' and args.few_shot != -1:
        calc_stats(examples, tokenizer, is_tokenize=True)
...
def load_and_cache_defect_data(args, filename, pool, tokenizer, split_tag, is_sample=False):
    ...
    elif (args.calibrate_cls or split_tag=='train') and args.few_shot != -1:
