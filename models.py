...
from models_list.ClassificationModelforCalibration import CloneModelforCalibration,DefectModelforCalibration
...
def bulid_or_load_cls_model(args):
    ...
    if args.task == 'defect':
        if args.calibrate_cls:
            model = DefectModelforCalibration(model, config, tokenizer, args)
        else:
            model = DefectModel(model, config, tokenizer, args)
    elif args.task == 'clone':
        # model.resize_token_embeddings(32000)
        if args.calibrate_cls:
            model = CloneModelforCalibration(model, config, tokenizer, args)
        else:
