...
from custom.calibration.ClassicalPLMCLSCalibrator import ClassicalPLMCLSCalibrator
...
def evaluate_cls(args, model, eval_examples, eval_data, write_to_pred=False):
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader), desc="Evaluating"):
        ...
        with torch.no_grad():
            if args.sub_task == "POJ":
                logits = model(inputs,p_inputs,n_inputs,label)
            else:
                logits = model(inputs, label) # logit shape:[batch_size]
            global calibrator
            if args.stage=='dev':
                calibrator.cat_tensor_to_variable(logits,'y_logits_dev')
                calibrator.cat_tensor_to_variable(label,'y_dev')
            elif args.stage=='test':
                calibrator.cat_tensor_to_variable(logits,'y_logits')
                calibrator.cat_tensor_to_variable(label,'y_true')
...
def main():
    ...
    logger.info("*********************************")
    if args.calibrate_cls:
        args.do_eval = True
        args.do_test = True
        global calibrator
        calibrator = ClassicalPLMCLSCalibrator(args)
    ...
    #             torch.cuda.empty_cache()
            ...
            for criteria in ['best-acc']:
                ...
                #     model = torch.nn.DataParallel(model)
                args.stage='dev'
                eval_examples, eval_data = load_and_cache_defect_data(args, args.dev_filename, pool, tokenizer, 'dev',is_sample=True)
            ...
            for criteria in ['best-f1']:
                ...
                #     model = torch.nn.DataParallel(model)
                args.stage='dev'
                eval_examples, eval_data = load_and_cache_clone_data(args, args.dev_filename, pool, tokenizer, 'dev',is_sample=True)
                result = evaluate_cls(args, model, eval_examples, eval_data)
        ...
        if args.task in ['summarize', 'translate', 'refine', 'generate','complete']:
            ...
            for criteria in ['best-f1']:
                ...
                eval_examples, eval_data = load_and_cache_clone_data(args, args.test_filename, pool, tokenizer, 'test',
                                                                    False)
                result = evaluate_cls(args, model, eval_examples, eval_data, write_to_pred=True)
    calibrator.write_pickled_data_to_file()
